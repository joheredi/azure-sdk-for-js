...
export type PurviewCatalog = Client & {
  path: Routes;
};
...
export default function PurviewCatalog(
  ...
  const baseUrl =
    options.baseUrl ||
    "https://{accountName}.catalog.purview.azure.com/api".replace(
      /{accountName}/g,
      accountName
    );
  options = {
  ...
  return getClient(baseUrl, credentials, options) as PurviewCatalog;
}

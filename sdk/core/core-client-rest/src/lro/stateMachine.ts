  ...
  switch (config.mode) {
    case "AzureAsync": {
      return processAzureAsyncOperationResult(
        createRetrieveAzureAsyncResource(sendOperationFn, args),
...
export function createGetLROState<TResult>(
  ...
  const poll = createPollingMethod(sendOperationFn, args, config.mode);
  const step = createTransition(sendOperationFn, args, config, finalStateVia);
  return async (path: string, pollerConfig: PollerConfig): Promise<LROState<TResult>> => {
    const response = step(await poll(path));
    if (retryAfter) throw new Error();
  };
...
export function initializeState<TResult>(
  ...
  state.initialRawResponse = response;
  state.isStarted = true;
  state.pollingURL = getPollingURL(state.initialRawResponse, path);

...
export class GenericPollOperation<TResult>
  ...
  constructor(
    ...
    private initialOperationArguments: RequestParameters,
    private sendOperation: SendOperationFn,
  ...
  async update(options?: {
    ...
    if (!state.isStarted) {
      const response = await this.sendOperation(this.initialOperationArguments);

      initializeState(state, this.initialPath, response);
    }
